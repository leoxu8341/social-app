...
const {
   letterListRequest, letterOneRequest, letterOpenRequest
} = letterAction;
...
const Inbox = (props) => {
   ...
   const { letters, loading, letterListRequest, letterOpenRequest, setSidebarKey } = { ...props };
         ...
         <Row>
            <Col span={24}>
               <List
                  ...
                  dataSource={letters && letters.items && letters.items.length > 0 ?
                  ...
                  renderItem={(item, index) => (
                     <List.Item>
                        <List.Item.Meta
                           avatar={
                              item.opened ? <Space > </Space> : <Space ><Badge color="red" /></Space> 
                           }
                           title={
                              <Button size="small" type="link" onClick={() => handleOnClick(item.key, item.opened)}>{item.subject}</Button>

                           }
                           description={
                              <div>
                                 {receiver ? <div><span style={{ marginRight: 20 }}>From: {item.sender_name}</span>
                                    <span style={{ marginRight: 20 }}>Received At: {item.created_at}</span></div>
                                  :
                                    <div> <span style={{ marginRight: 20 }}>To: {item.receiver_name}</span>
                                       <span style={{ marginRight: 20 }}>Sent At: {item.created_at}</span></div> }
                              </div>
                           }
                        />
                     </List.Item>
